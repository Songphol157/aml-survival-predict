...
# Data Cleaning - Chromosome data
def classify_karyo(val):
    if pd.isna(val):
        return np.nan
    s = str(val).lower().replace(' ', '')
    # match patterns like 46,xx[20] or 46,xy[20] or 46,xx[20] 46,xx[19]
    if all(re.fullmatch(r'46,(xx|xy)(\[\d+\])?', c) for c in re.split(r'[/ ]', str(val).lower().strip())):
        return 'normal'
    return 'complex'
...
X_full_cph = X_full.copy()
y_full_struct = Surv.from_arrays(event=y_event.to_numpy(dtype=bool), time=y_time)
X_full_cph['os_months'] = y_time
X_full_cph['event'] = y_event
# fit cph model, rsf and gbm to full data for coefficient extraction and to save the final model
cph_eln_cv.fit(X_full_cph, duration_col='os_months', event_col='event')
cph_cv.fit(X_full_cph, duration_col='os_months', event_col='event')
rsf_cv.fit(X_full, y_full_struct)
gbm_cv.fit(X_full, y_full_struct)
...
X_binary_full = X_binary_full.apply(pd.to_numeric, errors='coerce')
X_binary_scaled = X_binary_full.copy()
X_binary_scaled[num_cols] = scaler.fit_transform(X_binary_full[num_cols]) # fit scaler on full data for final model

# fit logistic regression, random forest, and xgboost on full data for coefficient extraction and to save the final model
lr_cv.fit(X_binary_scaled, y_binary)
rf_cv.fit(X_binary_full, y_binary)
xgb_cv.fit(X_binary_full, y_binary)
# fit MLP on full data for feature importance extraction and to save the final model
mlp_cv.fit(X_binary_scaled, y_binary)
...
print('\nComputing model coefficients / importances from full dataset...')

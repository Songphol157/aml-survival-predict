...
# Imputation for survival analysis (use mode imputer from training)
trained_imputer = joblib.load(os.path.join(MODELS_DIR, "mode_imputer.joblib"))
X_ext_imputed = trained_imputer.transform(X_ext)
...
# Time-dependent AUROC over months (use external data)
plt.figure(figsize=(8, 6))
plt.plot(times, auc_eln, label=f"ELN2017 Cox (Mean AUROC: {np.mean(auc_eln):.2f})", 
         lw=2, linestyle='--', color='gray')
for name, vals in aucs.items():
    plt.plot(times, vals, label=f"{name} (Mean AUROC: {np.mean(vals):.2f})", lw=2)
...
plt.tight_layout()
surv_roc_path = os.path.join(FIGURES_DIR, 'time_survival_roc_ext.jpg')
...
Xb_imputed = trained_imputer.transform(Xb)
...
if 'scaler' in models:
    ...
    Xb_scaled = Xb.copy()
    if 'lr' in models or 'mlp' in models:
        print("Warning: Logistic Regression or MLP model found but scaler is missing. Running these models on unscaled data.")
